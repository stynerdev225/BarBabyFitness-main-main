...
// Configure Cloudflare R2 client to read templates directly
...
// Function to check if we can read templates from R2
async function verifyR2Templates() {
  try {
    console.log('Verifying R2 templates access...');
    ...
    let allTemplatesAccessible = true;
    ...
    for (const templateKey of templates) {
      ...
      try {
        ...
        console.log(`✅ Template accessible: ${templateKey} (${response.ContentLength} bytes)`);
      } catch (error) {
        console.error(`❌ Cannot access template ${templateKey}:`, error.message);
        allTemplatesAccessible = false;
      }
    }
    
    return allTemplatesAccessible;
  } catch (error) {
    console.error('Error verifying R2 templates:', error);
    return false;
  }
}
...
async function testContractGeneration() {
  console.log('\nTesting contract generation with sample data...');
  
  try {
    // First verify we can access R2 templates
    const templatesOk = await verifyR2Templates();
    if (!templatesOk) {
      console.warn('⚠️ Warning: Not all templates are accessible in R2. Test may fail.');
    }

...
const transporter = {
  sendMail: (options) => {
    return new Promise((resolve) => {
      console.log(`MOCK EMAIL: Would send from "${options.from}" to "${options.to}" with subject "${options.subject}"`);
      resolve({ 
      ...
      <div class="email-container">
        ...
        <div class="footer">
          ...
          <p>
            ...
            <a href="https://barbabyfitness.com">Website</a>
          </p>
        </div>
      </div>
    </body>
...
router.post('/post-payment-actions', async (req, res) => {
  ...
  try {
    ...
    // Try to fetch contract documents from Cloudflare R2
    let attachments = [];
    try {
      if (contractUrl && contractUrl.includes('filled-contracts')) {
        console.log('Contract URL appears to be a filled PDF, attempting to fetch documents');
        
        // Extract the client name and timestamp from the contract URL
        const urlParts = contractUrl.split('/');
        const fileName = urlParts[urlParts.length - 1];
        const matches = fileName.match(/contract_(.+?)_(\d+)\.pdf/) || 
                       fileName.match(/(.+?)_(\d+)\.pdf/);
        
        if (matches && matches.length >= 3) {
          const extractedClientName = matches[1];
          const timestamp = matches[2];
          
          console.log(`Extracted client name: ${extractedClientName}, timestamp: ${timestamp}`);
          
          // Look for all documents with this client name and timestamp
          const documentTypes = ['contract', 'registration', 'waiver'];
          
          for (const docType of documentTypes) {
            const docKey = `filled-contracts/${docType}_${extractedClientName}_${timestamp}.pdf`;
            
            try {
              console.log(`Looking for document: ${docKey}`);
              ...
              if (response.Body) {
                ...
                attachments.push({
                  filename: `${docType.charAt(0).toUpperCase() + docType.slice(1)}.pdf`,
                  content: pdfBuffer,
                  contentType: 'application/pdf'
                });
                
                console.log(`Added attachment for ${docType}`);
              }
            } catch (error) {
              console.log(`Could not find document: ${docKey}`);
            }
          }
        } else {
          console.log('Could not parse client name and timestamp from URL:', contractUrl);
        }
      } else {
        console.log('Contract URL is not a filled PDF or is missing');
      }
    } catch (error) {
